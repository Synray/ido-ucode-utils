...
void readuinstr(union Bcode *bcode, char *ustr) {
...
#if 0
    //printf("%.4s[%d] ", urec.opcname, instlength);
    if (uinstr->Opc == Uent) puts("");
    printf("%.4s ", urec.opcname);

    char dt = dtype_name(uinstr->Dtype);
    char mt = mtype_name(uinstr->Mtype);
    printf("(%cdt ", dt);
    if (dt == 'U')
    {
        printf ("%hhu ", uinstr->Dtype);
    }
    printf("%cmt", mt);
    if (mt == 'U')
    {
        printf (" %hhu", uinstr->Mtype);
    }
    printf(" LL %hu", uinstr->Lexlev);
    printf(" I1 %d)", uinstr->I1);
#endif
    ...
    if (instlength > 2)
    {
        ...
        switch (uinstr->Opc) {
            case Uent:
            case Ucup:
                printf("pop %u push %u external %u ", uinstr->UENT.Push, uinstr->UENT.Pop, uinstr->UENT.Extrnal);
                break;
            case Ucvt:
                {
                    dt = dtype_name(uinstr->UCVT.secondty.Dtype2);
                    printf("DT2 ");
                    printf("%cdt", dt);
                    if (dt == 'U')
                    {
            ...
            default:
                printf("len %d off %d ", uinstr->UIEQU.Length, uinstr->UIEQU.offset);
                break;
        }
    }
    ...
    if (instlength > 4)
    {
        for (i = 4; i < instlength; i += 2) {
            printf("%d %d ", bcode->intarray[i], bcode->intarray[i + 1]);
        }
    }
#endif
...
#if 0
            printf("- string: %.*s", strlength*4, ustr);
#endif
        }
    }
#if 0
